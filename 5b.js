...
let curDate = {time:Date.now()}
Object.freeze(curDate);
...
function drawMenu() {
	...
	drawMenu0Button('SIGN IN', 665.55, 437.7, userInfo != null, eventLogin);
	drawMenu0Button('START ROUND!', 665.55, 487.7, userInfo == null, menuLevelCreator);
...
function draw() {
	...
	switch (menuScreen) {
		...
		case 6:
			...
			// if (enableExperimentalFeatures) drawMenu2_3Button(2, 10, 486.95, eventLogin);
			if (loggedInExploreUser5beamID === -1) {
				drawMenu0Button('LOG IN', 540, 20, false, eventLogin, 120);
			} else {
...
function getAuthHeader() {
	return {'Authorization': 'pb_auth=' + localStorage.getItem('mo5s_auth')};
}
...
function getCurrentExploreUserID() {
	...
	return fetch("http://runasbotserver.my.pebble.host:25637/getStuff", {method: "GET", headers: getAuthHeader()})
		.then(async response => {
...
function eventLogin() {
	...
	newWindow = window.open(
		'http://runasbotserver.my.pebble.host:25637/redir',
		'MO5S Login',
